...
#==========================================================
# URL to the microservice running as an AWS Lambda.
#
# IMPORTANT: You must adjust it to the correct AWS Lambda
# gateway.
#
URL_MICROSERVICE_SCORES = 'https://e0oc9i7qnf.execute-api.us-west-2.amazonaws.com/default/scores'
...
def ask_questions(n)
  ...
  if score > 0
    ...
    RESTful.post(URL_MICROSERVICE_SCORES,
      {'initials' => initials.upcase, 'score' => score}.to_json,
      'application/json')
  end
  ...
  puts '--------- ----- -------------------'
  scores.each do |s|
    puts '%-10s  %3d %s' % [s['initials'], s['score'], s['timestamp'][0..18]]
  end
  puts '--------- ----- -------------------'
end
